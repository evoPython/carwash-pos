document.addEventListener("DOMContentLoaded", () => {
    ...
    // Check for shift summary on page load
    checkShiftSummaryExists();
    ...
    if (form) {
        form.addEventListener("submit", async e => {
            ...
            if (r.ok) {
                ...
                showError("Order Failed", "Failed to add order: " + (error.error || "Unknown error"));
            }
        });
    }
    ...
    function updateShiftTimer() {
        ...
        if (userRoleElement && userRoleElement.textContent === 'Incharge') {
            if (userShift === 'AM') {
                ...
                showTemporaryButton = (currentHour === 16 && currentMinutes >= 30) || currentHour === 17;
            } else if (userShift === 'PM') {
                // For PM shift, show from 4:30 AM to 5:00 AM
                showTemporaryButton = (currentHour === 4 && currentMinutes >= 30) || currentHour === 5;
            }
        }
        ...
        if (timerElement) {
            if (isWithinShift) {
                ...
                // Order button logic - always visible during active shift
                if (btnOpen) {
                    btnOpen.style.display = 'inline-block';
                }

                // Temporary button logic - only for InCharges 30 minutes before shift ends
                if (userRoleElement && userRoleElement.textContent === 'Incharge') {
                    if (btnTemporary) {
                ...
                // Hide both buttons when shift is inactive
    ...
    function updateOtherIncomeTotal() {
        ...
        if (otherIncomeTotalElement) {
            otherIncomeTotalElement.remove();
        }
    ...
    function updateGrandTotal() {
        ...
        const grandTotal = totalSales + totalOtherIncome - totalExpenses - gcash - 400;
    ...
    // Add event listener for GCash input
    const gcashInput = document.getElementById('gcash');
    if (gcashInput) {
        gcashInput.addEventListener('input', updateGrandTotal);
    }

    // Add event listener for wages input
    const wagesInput = document.getElementById('wages');
    if (wagesInput) {
        wagesInput.addEventListener('input', updateGrandTotal);
    }
    ...
    if (temporaryModalSubmit) {
        temporaryModalSubmit.addEventListener('click', async function(e) {
            ...
            try {
                console.log("Starting shift summary submission...");

                // Check if summary already exists before submission
                if (summaryAlreadyExists) {
                    showWarning('Already Submitted', 'This shift summary has already been completed.');
                    return;
                }
                ...
                formData.grand_total = totalSales + totalOtherIncome - totalExpenses - formData.gcash - formData.wages;

                console.log("Final form data to submit:", formData);

                // Add confirmation prompt
                const isConfirmed = window.confirm("Are you sure? Changes cannot be undone.");
                if (!isConfirmed) {
                ...
                if (response.ok) {
                    ...
                    showSuccess("Summary Submitted", `Successfully submitted shift summary for ${result.date} (${result.shift} shift)`);
                    ...
                    // Set the flag to prevent future submissions
                    summaryAlreadyExists = true;
                ...
                showError("Submission Error", "An error occurred while submitting the shift summary. Please try again.");
                if (temporaryModal) {
                    temporaryModal.style.display = 'none';
                }
            }
    ...
    // ====================================
    // SUMMARY FORM LOGIC ENHANCEMENTS
    // ====================================
    
    let summaryAlreadyExists = false;

    // Check if shift summary already exists
    async function checkShiftSummaryExists() {
        try {
            console.log("Checking if shift summary already exists...");
            const response = await fetch('/api/shift-summary-exists');
            console.log("Shift summary API response:", response);
            if (response.ok) {
                const result = await response.json();
                console.log("Shift summary check result:", result);
                summaryAlreadyExists = result.exists;

                if (summaryAlreadyExists) {
                    console.log("Shift summary already exists, hiding button");
                    // Hide temporary button if summary already exists
                    const btnTemporary = document.getElementById('btn-temporary');
                    if (btnTemporary) {
                        btnTemporary.style.display = 'none';
                    }
                    ...
                    showInfo('Summary Complete', `Shift summary for ${result.date} (${result.shift} shift) has already been completed.`);
                } else {
                    console.log("No existing shift summary found");
                }
            } else {
                console.log("No existing shift summary (API returned non-ok status)");
            }
        } catch (error) {
            console.error('Error checking shift summary:', error);
        }
    }

    // Enhanced updateShiftTimer function to check summary status
    const originalUpdateShiftTimer = window.updateShiftTimer;
    window.updateShiftTimer = function() {
        // Call original function
        if (originalUpdateShiftTimer) {
            originalUpdateShiftTimer();
        }

        // Check summary status when timer updates
        const userRoleElement = document.querySelector(".user-role");
        if (userRoleElement && userRoleElement.textContent === 'Incharge') {
            checkShiftSummaryExists();
        }
    ...
    // Enhanced temporary modal submit to hide button after submission
    const originalTemporarySubmit = document.getElementById('temporary-modal-submit');
    if (originalTemporarySubmit) {
        originalTemporarySubmit.addEventListener('click', async function(e) {
            ...
            // After successful submission, hide the temporary button permanently
            setTimeout(() => {
                const btnTemporary = document.getElementById('btn-temporary');
                if (btnTemporary) {
                    btnTemporary.style.display = 'none';
                ...
                // Hide the modal
                if (document.getElementById('temporary-modal')) {
                    document.getElementById('temporary-modal').style.display = 'none';
                }
